...
describe("Website Field Directive", function() {
  ...
  it("It should be an website element", function() {
    should(this.element.find('input').attr('type')).equal('url');
  });
  ...
  it("Setting a value on the scope should give set a default value", function() {
    var self = this;
    self.fieldWebsiteCtrl.field.value = 'http://redhat.com';
    self.scope.$apply();

    should(self.element.find('input').val()).equal('http://redhat.com');
  });
